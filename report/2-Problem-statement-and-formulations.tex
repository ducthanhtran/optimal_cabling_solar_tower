...
\newcommand*{\assignData}{a_{\text{data}}}
\newcommand*{\assignPower}{a_{\text{power}}}
\begin{tcolorbox}
\label{prob:cabling_problem}
\begin{problem}[Cabling Problem]
  Given a complete, directed graph $G=(V,A)$, a set of data cables $D$ and power cables $P$. Two directed, rooted trees $\treeData,\treePower\subseteq G$ with cable assignments $\assignData:A(\treeData)\to D$ and \\
  $\assignPower: A(\treePower)\to P$, where the root is the solar tower $T\in V$, have to be constructed such that the following properties are fulfilled:
  \begin{description}
    \item[Data cable] asd
    \item[Power cable] asd
