\section{Heuristics}
We only consider fiberglass cables for the data connections for simplicity due to their nature of being unrestricted in both length and capacity. Moreover, we compute solutions for which both data and power cable are installed 
\emptyline
...
Because the largest power cable has a capacity of 250 heliostats and instances like the PS10 have more heliostats we have to use additional power cables in order to connect all heliostats, hence a partioning of the heliosats has to be considered in addition. We perform a simple heuristic for this problem by computing the angles $\alpha_h$ between the reference vector $(1, 0)^T$ and a heliostat $h\in V_H$ like in \cref{fig:partioning}. Afterwards, the partioning is the result of sorting the angles and splitting them according to a parameter $K\in\mathbb{N}$. All in all, this procedure has a time complexity of $\mathcal{O}\left(|V_H|^2\log\left(|V_H|\right)\right)$.

Important to note is that we can compute initial solutions for the local search procedures independently on each partition, therefore obtaining $K$ initial solutions for $K$ partitions.
\vspace*{1.8cm}
\begin{figure}[H]
    \begin{center}
    \begin{tikzpicture}
...
\begin{figure}[H]
  \center
    ...
    \end{tikzpicture}
  \caption{Consider the cycle neighborhood $N(B)$ for some spanning tree $B\subset G$, that is, delete edge $e$ and reconnect the resulting partitions $P_1$ and $P_2$ by adding an edge $f$ with $c(f) < c(e)$, yielding a spanning tree with lower costs.}
  \label{fig:local_search_mst_step}
\end{figure}
\noindent
Moreover, due to changes in capacities we also need to consider the power cabling in the local search improvement step. Let us take up the case of deleting the edge $(u,w)\in E(B)$ in our current spanning tree $B\in\mathcal{B}$. Moreover, the edge $(v,w)\notin E(B)$ is considered as replacement. This procedure is sketched in \cref{fig:upgrade_downgrade}.
\begin{figure}[H]
    \begin{tikzpicture}[node distance=1cm]
    ...
    \node[vertex,fill,orange,text=black] (q3)[above left=1cm and 3cm of q2]{$u$};
    ...
    \matrix [left=1cm of v]  {
      \node[label=right:Descendants of $w$, vertex,fill=black!60!green,minimum size=0.3cm] [] {}; \\
      \node[label=right:Ancestors of $w$, vertex,fill,orange,minimum size=0.3cm] {}; \\
      \node[label=right:Ancestors of $v$, vertex,fill,blue,minimum size=0.3cm] {}; \\	
    };
    ...
    \end{tikzpicture}
  \caption{For power cables we need to consider up- and downgrades in each improvement step, consequently potential savings by switching to lower-quality cables are made.}
  \label{fig:upgrade_downgrade}
\end{figure}
\noindent
...
For each edge $(r,s)\in path(T,u)$ we check whether we can switch from $\assignPower((r,s))\in P$ to a cheaper power cable $p'\in P$ such that
\begin{itemize}
  \item $|descendants(s)| + 1 \leq maxCapacity(p')$
  \item $d(r,s) \leq maxLength(p')$
\end{itemize}

\subsubsection{Upgrade}
If the additional capacity cannot be satisfied by the current power cable $\assignPower((r,s))$ for an edge $(r,s)\in path(T,v)$ we need to upgrade to a higher-capacity cable $p'\in P$ such that
\begin{itemize}
  \item $|descendants(s)| + 1 \leq maxCapacity(p')$
  \item $d(r,s) \leq maxLength(p')$
\end{itemize}
...
For each partition $k=1,\ldots,K$ we obtain a hamiltonian path $H_k\subset G_k$. Now, consider moving a vertex from a hamiltonian path $H_i$ to a path $H_j$ for $1\leq i,j\leq K$. In particular consider the graph construct in \cref{fig:hamiltonian_path_local_search_start} where $v,w\in V(H_i)$ and $x,y,z \in V(H_j)$.
\begin{figure}[H]
\centering
...
\end{tikzpicture}
\caption{Starting point of the local search step. We denote the left hamiltonian path that contains the vertices $v$ and $w$ as $H_i$, the right one with $H_j$.}
...
\begin{align*}
  ...
  -&c(v,w) - c(x,y) - c(x,z)
\end{align*}
In the case of $z$ being non-existent we leave out the terms $c(y,z)$ and $c(x,z)$. We further introduce a threshold $Q\in\mathbb{Q}^{-}$ and perform the local search in a first-improvement fashion. That is, if we encounter a configuration with $R \leq Q$ then we update our solution by moving $x$ to the respective hamitonian path.
This step is repeated until no configuration exists for which the threshold $Q$ is not fullfilled, therefore having a locally optimal solution. The threshold $R$ is introduced to accelerate termination of the algorithm and is set $Q:=-1.0$ in our work, in other words we only perform the improvement if a newly found configuration is at least 1\euro\ better in terms of cost.
\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=3cm]
\tikzstyle{vertex}=[draw,shape=circle,minimum size=1cm];
