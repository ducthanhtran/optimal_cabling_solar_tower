...
\begin{equation*}
  N(B) := \{\tilde{B}\in\mathcal{B}\ |\ c(\tilde{B}) < c(B) \land |E(\tilde{B})\setminus E(B)| = 1 \}
\end{equation*}
for a spanning tree $B\subset G_k$ and $\mathcal{B}$ denotes the set of all spanning trees in $G_k$.

For an update step we thus delete an edge $e\in E(B)$ in our current tree $B$ and reconnect the disconnected partition by a new edge $f\in E\setminus{(E(B)\cup\{e\})}$ which yields $\tilde{B}$. By doing this we effectively move from $B$ to another spanning tree $\tilde{B}$ in the search space. In order to improve our current solution we always choose an edge $f$ with $c(f) < c(e)$. If no such edge exists, we can terminate the local search procedure as the neighborhood $N(B)$ is empty, hence $B$ is a locally optimal solution. Such an update step is sketched in \cref{fig:local_search_mst_step}.
\begin{figure}[H]
  \center
